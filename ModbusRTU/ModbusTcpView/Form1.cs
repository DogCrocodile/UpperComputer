...
namespace WindowsFormsApp1
{
    ...
    public partial class Form1: Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            IsConnected =  modbusTCP.Connect(this.IpText.Text.Trim(), Convert.ToInt32(this.portText.Text.Trim()));
            if (IsConnected)
            {
                AddLog("Modbus连接成功。");
                return;
        ...
        private bool WirteBool(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            switch (storeArea)
            {
                case StoreArea.输入线圈1x:
                    ...
                    if(values.Length == 1)
                    {
                        result = modbusTCP.PreSetSingleCoil(start, values[0]);
                    }
                    else
                    {
                        result = modbusTCP.PreSetMultiCoils(start, values);
                    }
                    break;
                case StoreArea.输出线圈0x:
                case StoreArea.输出寄存器4x:
        ...
        private bool WirteByte(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            {
                case StoreArea.输出寄存器4x:
                    result = modbusTCP.PreSetMultiRegisters(start, ByteArrayLib.GetByteArrayFromHexString(setValue));
                    break;
                case StoreArea.输入线圈1x:
                case StoreArea.输出线圈0x:
                case StoreArea.输入寄存器3x:
        ...
        private bool WirteShort(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            switch (storeArea)
            {
                case StoreArea.输出寄存器4x:
                    if(values.Length == 1)
                    {
                        result = modbusTCP.PreSetSingleRegister(start, values[0]);
                    }
                    else
                    {
                        result = modbusTCP.PreSetMultiRegisters(start, ByteArrayLib.GetByteArrayFromShortArray(values));
                    }
        ...
        private bool WirteUShort(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            switch (storeArea)
            {
                case StoreArea.输出寄存器4x:
                    ...
                    {
                        result = modbusTCP.PreSetMultiRegisters(start, ByteArrayLib.GetByteArrayFromUShortArray(values));
                    }
        ...
        private bool WirteInt(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            switch (storeArea)
            {
                case StoreArea.输出寄存器4x:
                    ...
                    result = modbusTCP.PreSetMultiRegisters(start, ByteArrayLib.GetByteArrayFromIntArray(values));
                    break;
                case StoreArea.输入线圈1x:
                case StoreArea.输出线圈0x:
                case StoreArea.输入寄存器3x:
        ...
        private bool WirteUInt(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            switch (storeArea)
            {
                case StoreArea.输出寄存器4x:
                    ...
                    result = modbusTCP.PreSetMultiRegisters(start, ByteArrayLib.GetByteArrayFromUIntArray(values));
                    break;
                case StoreArea.输入线圈1x:
                case StoreArea.输出线圈0x:
                case StoreArea.输入寄存器3x:
        ...
        private bool WirteFloat(StoreArea storeArea, byte salveId, ushort start, string setValue)
        {
            ...
            switch (storeArea)
            {
                case StoreArea.输出寄存器4x:
                    ...
                    result = modbusTCP.PreSetMultiRegisters(start, ByteArrayLib.GetByteArrayFromFloatArray(values));
                    break;
                case StoreArea.输入线圈1x:
                case StoreArea.输出线圈0x:
                case StoreArea.输入寄存器3x:
